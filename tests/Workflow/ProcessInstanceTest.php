...
use Yummuu\Workflower\Workflow\Activity\WorkItem;
...
use Yummuu\Workflower\Workflow\Provider\DataProviderInterface;
...
class ProcessInstanceTest extends TestCase
{
    ...
    public function start()
    {
        ...
        $this->assertThat($currentFlowObject, $this->isInstanceOf('Yummuu\Workflower\Workflow\Activity\ActivityInterface'));
        ...
        $this->assertThat($previousFlowObject, $this->isInstanceOf('Yummuu\Workflower\Workflow\Event\StartEvent'));
    ...
    public function getActivityLog()
    {
        ...
        $this->assertThat($activityLog, $this->isInstanceOf('Yummuu\Workflower\Workflow\ActivityLogCollection'));
    ...
    public function executeServiceTasks()
    {
        ...
        $operationRunner->expects($this->exactly(2))->method('run')->willReturnCallback(function (WorkItemInterface $workItem) use ($self) {
            ...
            $self->assertThat($operational, $self->isInstanceOf('Yummuu\Workflower\Workflow\Activity\ServiceTask'));
    ...
    public function executeSendTasks()
    {
        ...
        $operationRunner->expects($this->exactly(2))->method('run')->willReturnCallback(function (WorkItemInterface $workItem) use ($self) {
            ...
            ++$calls;
            $operational = $workItem->getActivity();
            $self->assertThat($operational, $self->isInstanceOf('Yummuu\Workflower\Workflow\Activity\SendTask'));
            $self->assertThat($operational->getOperation(), $self->equalTo('phpmentors_workflower.service'.$calls));
            $self->assertThat($operational, $self->isInstanceOf('Yummuu\Workflower\Workflow\Resource\MessageInterface'));
